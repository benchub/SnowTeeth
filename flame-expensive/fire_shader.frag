...
void main() {
    ...
    // 35-iteration raymarch loop (optimized for performance)
    for(float step = 0.0; step < 35.0; step++) {
        ...
        // - turbulence: displacement amplitude
        // - frequency: wavelength (spacing between kinks)
        // - detail: octave scaling (low=smooth, high=intricate)
        float freq = u_frequency;
        float baseAmplitude = u_turbulence * u_frequency;
        float octaveFactor = mix(0.75, 0.45, (u_detail - 0.5) / 1.5);
        // Reduced from 8 to 6 octaves for performance
        for(int turbLoop = 0; turbLoop < 6; turbLoop++) {
